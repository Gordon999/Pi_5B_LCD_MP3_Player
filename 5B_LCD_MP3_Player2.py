...
while True:
    ...
    while MP3_Play == 0 and radio == 0:
        time.sleep(0.1)
        if lcd_lines == 2:
            if bl_on == 1 and album == 0 and len(tracks) > 0:
                titles[0],titles[1],titles[2],titles[3],titles[4],titles[5],titles[6] = tracks[Track_No].split("/")
        ...
        if  button_next.is_pressed and len(tracks) > 0:
            ...
            old_album = titles[1]
            old_artist = titles[0]
            lcd.text(str(Track_No+1) + ": " + titles[0][0:16 - len(str(Track_No+1))], 1)
            time.sleep(0.05)
            lcd.text(titles[1][0:16], 2)
            if lcd_lines == 4:
                lcd.text("",3)
            ...
            while button_prev.is_pressed:
                ...
                if time.monotonic() - timer3 > 2:
                  if button_play.is_pressed:
                    ...
                    old_album  = titles[1]
                    old_artist = titles[0]
                    lcd.text(str(Track_No+1) + ": " + titles[4][0:16 - len(str(Track_No+1))], 1)
                    time.sleep(0.05)
                    lcd.text(titles[1][0:16], 2)
                    if lcd_lines == 4:
                        lcd.text("",3)
